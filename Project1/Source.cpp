...
#pragma region 인라인 함수
	...
	// 복사되기 때문에 실행 파일의 크기가 커지게 된다.

	// Information();

	// 인라인 함수의 경우 컴파일 시점에 확정되며, 컴파일 시 인라인
	// 함수로 선언하더라도 상황에 따라 일반 함수로 변환되기도 한다.
...
#pragma region 정수의 승격
	...
	// 작은 자료형은 int 자료형으로 변환하는 것

	// char a = 10;	// 1 byte
	// short b = 20;	// 2 byte

	// printf("a 변수와 b 변수를 더한 메모리 크기 : %d byte\n"
	//	, sizeof(a + b));
#pragma endregion
...
#pragma region 부호없는 자료형
	...
	// printf("unAlphabet 변수의 값 : %u", unAlphabet);
